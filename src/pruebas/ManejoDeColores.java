...
public class ManejoDeColores {
	...
	public void agregarColor(Color col){
		...
		if (diferencia(col.getRed(),colorDeFondo.getRed()) <= epsilon)
			if (diferencia(col.getGreen(),colorDeFondo.getGreen()) <= epsilon)
				//if (diferencia(col.getBlue(),colorDeFondo.getBlue()) <= epsilon)
					colores.add(col);
	...
	public void marcarObjeto(int i, int j, int offset, int nbands){
		...
		if (count+3 < maxCount && count > 0)
			if (i-1 > 0 && i+1 < width && j-1 > 0 && j+1 < height){
				if(matriz[i][j] == 0){
					if (!isAzulCompleto(i, j, offset, nbands)){
						matriz[i][j]= 1;
						setData(count,(byte)255);
						if ((i+1 < width)&&(matriz[i+1][j] == 0)){
								if (!isAzulCompleto(i+1, j, offset, nbands)){
									marcarObjeto(i+1, j,  offset, nbands);
								}else{
									matriz[i+1][j]= -1;
						...
						if ((j+1 < height)&&(matriz[i][j+1] == 0)){
							if (!isAzulCompleto(i, j+1, offset, nbands)){
								marcarObjeto(i, j+1,  offset, nbands);
							}else{
								matriz[i][j+1]= -1;
	...
	public int detectarObjeto(int w, int h, int offset, int nbands, int R, int G, int B){
